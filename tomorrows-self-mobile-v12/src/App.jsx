  ...
  return (
    <div className={`phone-app ${sanctuaryState.toLowerCase()}`}>
      ...
      <main>
        ...
        {tab === "profile" && <Profile rank={rank} nextRank={nextRank} rankProgress={rankProgress} xp={xp} flamePower={flamePower} sound={sound} setSound={setSound}/>}
      </main>
...
function Sanctuary({ quote, quoteIndex, setQuoteIndex, flamePower, sanctuaryState, rank, nextRank, rankProgress, xp, entry, updateEntry, saveCheckIn, useGrace, goals, toggleGoal, setTab, sound, setSound }) {
  return <div className="stack">
    ...
    <Card>
      ...
      <div className="sound-grid">
        {ambience.map(({ name, icon: Icon }) => <button className={sound === name ? "active" : ""} key={name} onClick={() => setSound(name)}><Icon/> {name}</button>)}
      </div>
    </Card>
  </div>
}
...
function Calendar({ historyDays }) {
  return <div className="mini-calendar">{historyDays.map(d => <div className={d.entry?.checkedIn ? "cal-day saved" : "cal-day"} key={d.key}><small>{d.day}</small><b>{d.num}</b><span>{d.entry?.mood ? moods.find(m => m.value === d.entry.mood)?.icon : "—"}</span></div>)}</div>
}
...
function Profile({ rank, nextRank, rankProgress, xp, flamePower, sound, setSound }) {
  return <div className="stack">
    ...
    <Card><h3>Ambience</h3><div className="sound-grid">{ambience.map(({name, icon: Icon}) => <button className={sound === name ? "active" : ""} key={name} onClick={() => setSound(name)}><Icon/> {name}</button>)}</div></Card>
