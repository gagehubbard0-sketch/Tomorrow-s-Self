...
import {
  ...
  Zap, HeartPulse, Sun, Sword, Activity, Clock, Star, Gift, Smile, Crosshair, Brain, HandHeart, Eye, Scale, Sprout
} from "lucide-react";
import "./styles.css";
...
export default function App() {
  ...
  const [communityPosts, setCommunityPosts] = useState(() => load("ts22_posts", posts));
  ...
  function toggleGoal(id) {
    ...
    setDone(next);
    save("ts20_done", next);
    if (next.length === goals.length) notify("All goals complete. The flame grows stronger.");
  }
  ...
  function submitPost() {
    ...
    const next = [{ name: "ShadowSeeker", tag: "Check-In", text: postText.trim(), fire: 0, comments: 0, avatar: "🛡️" }, ...communityPosts];
    setCommunityPosts(next);
    save("ts22_posts", next);
    setPostText("");
    notify("Posted to the community feed.");
  }
  ...
  return (
    <div className="app">
      ...
      <main className="main">{toast && <div className="toast">{toast}</div>}
        ...
        <section className="hero">
          <div className="scene"><img className="heroBgImage" src={heroBackground} alt="cinematic field and warrior" />
        ...
        <section className="content">
          <div className="leftContent">
            ...
            <div className="middleGrid">
              ...
              <Card className="actionsPanel">
                <h3>Quick Actions</h3>
                {[
                  [Crosshair, "Start Focus Session", () => startSound("Temple")],
                  [Dumbbell, "Log Workout", () => toggleGoal("workout")],
                  [HeartPulse, "Log Meal", () => markStreak("health")],
                  [ScrollText, "Write in Journal", () => setActive("Journal")],
                  [Moon, "Evening Reflection", () => setActive("Journal")]
                ].map(([Icon, label, action]) => <button key={label} onClick={action}><Icon size={21}/>{label}</button>)}
              </Card>
            </div>

            <div className="bottomGrid">
              <Card className="journalPanel">
                <div className="sectionHead"><h3>Recent Journal</h3><button onClick={() => setActive("Journal")}>View All ›</button></div>
            ...
            <Card className="growthGuide">
              <div className="sectionHead">
                <h3>Humility, Wisdom & Peace</h3>
                <button>Daily Growth</button>
              </div>

              <div className="guideGrid">
                <div className="guideColumn">
                  <div className="guideTitle"><HandHeart size={22}/><h4>Practice Humility</h4></div>
                  <ul>
                    <li><b>Admit Mistakes & Limits:</b> Recognize when you are wrong or limited.</li>
                    <li><b>Listen & Learn:</b> Treat conversations as chances to learn, not prove yourself right.</li>
                    <li><b>Avoid Self-Elevation:</b> Stop comparing yourself through superiority or inferiority.</li>
                    <li><b>Practice Gratitude:</b> Notice the gifts you have and the help you receive.</li>
                    <li><b>Laugh at Yourself:</b> Build humor about your own failings.</li>
                    <li><b>Appreciate Others' Gifts:</b> Celebrate talents different from your own.</li>
                  </ul>
                </div>

                <div className="guideColumn">
                  <div className="guideTitle"><Brain size={22}/><h4>Become Wiser</h4></div>
                  <ul>
                    <li><b>Beginner's Mind:</b> Live like there is always more to learn.</li>
                    <li><b>Intellectual Humility:</b> Remember your knowledge is limited.</li>
                    <li><b>Ask Why and How:</b> Understand deeper causes instead of only reacting.</li>
                    <li><b>Seek Understanding:</b> Choose empathy over winning arguments.</li>
                  </ul>
                </div>

                <div className="guideColumn">
                  <div className="guideTitle"><Sprout size={22}/><h4>Become More Peaceful</h4></div>
                  <ul>
                    <li><b>Let Go of Ego:</b> Pride creates tension; humility calms the room.</li>
                    <li><b>Practice Acceptance:</b> Accept people and situations before responding.</li>
                    <li><b>Control Your Internal State:</b> Keep clean thoughts and release old hurts.</li>
                    <li><b>Let Go of Judgment:</b> Release the need to judge every different approach.</li>
                    <li><b>Service & Purpose:</b> Help others and live for something deeper.</li>
                  </ul>
                </div>
              </div>

              <div className="dailyPractice">
                <div><Eye size={20}/><b>Daily Reflection</b><span>Did I act with humility and kindness today?</span></div>
                <div><Scale size={20}/><b>Assume Good Intentions</b><span>Give others the benefit of the doubt.</span></div>
                <div><Sparkles size={20}/><b>The Mirror Check</b><span>Be humble. Be gentle. Keep growing.</span></div>
              </div>
            </Card>
          </div>
          ...
          <Card className="communityPanel">
            ...
            <input className="postInput" value={postText} onChange={e => setPostText(e.target.value)} onKeyDown={e => e.key === "Enter" && submitPost()} placeholder="Share something, stay accountable." />
            <div className="anon"><span></span> Anonymous <button onClick={submitPost}>Post</button></div>
        ...
        {active !== "Today" && (
          <section className="pagePanel">
            <button className="backBtn" onClick={() => setActive("Today")}>← Back to Dashboard</button>
            <h2>{active}</h2>
            {active === "Goals" && <Card className="pageCard"><h3>Daily Goals</h3><div className="goalList">{goals.map(g => <button key={g.id} className={done.includes(g.id) ? "goal done" : "goal"} onClick={() => toggleGoal(g.id)}><span>{done.includes(g.id) ? <CheckCircle2 size={18}/> : <Circle size={18}/>}</span><em>{g.icon}</em><b>{g.title}</b></button>)}</div><button className="addGoal" onClick={addGoal}><Plus size={18}/> Add Goal</button></Card>}
            {active === "Journal" && <Card className="pageCard"><h3>Journal</h3><textarea className="bigText" value={journal} onChange={e => { setJournal(e.target.value); save("ts20_journal", e.target.value); }} /><button className="addGoal" onClick={() => notify("Journal saved.")}>Save Reflection</button></Card>}
            {active === "Streaks" && <Card className="pageCard"><h3>Streaks</h3><div className="streakGrid">{streaks.map((s) => { const Icon = s.icon; return <div className="streakBox" key={s.id}><Icon className={`streakIcon ${s.tone}`} size={44}/><h4>{s.title}</h4><div><b>{s.days}</b><span>days</span></div><p>{s.sub}</p><Progress value={s.progress} tone={s.tone}/><button onClick={() => markStreak(s.id)}>Mark today</button></div>})}</div></Card>}
            {active === "Community" && <Card className="pageCard"><h3>Community</h3><input className="postInput" value={postText} onChange={e => setPostText(e.target.value)} onKeyDown={e => e.key === "Enter" && submitPost()} placeholder="Share something, stay accountable." /><button className="addGoal" onClick={submitPost}>Post</button><div className="posts">{communityPosts.map(post => <article key={post.name + post.text}><div className="postAvatar">{post.avatar}</div><div><div className="postHead"><b>{post.name}</b><small>now</small><em>{post.tag}</em></div><p>{post.text}</p></div></article>)}</div></Card>}
            {active === "Insights" && <Card className="pageCard"><h3>Insights</h3><div className="insightGrid"><div><Smile/><small>Mood Average</small><b>Steady</b></div><div><CheckCircle2/><small>Goals Completed</small><b>{questPct}%</b></div><div><Clock/><small>Focus Time</small><b>12.4h</b></div><div><Star/><small>Best Day</small><b>May 8</b></div></div></Card>}
            {(active === "Health" || active === "Quests" || active === "Profile" || active === "Settings") && <Card className="pageCard"><h3>{active}</h3><p>This section opens correctly now. We can build it out without changing the dashboard design.</p></Card>}
          </section>
        )}
